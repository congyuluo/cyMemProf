import re
from globals import c_source_loc, c_altered_loc, c_processed_loc, c_processed_suffix, c_inst_counting_suffix, c_scope_enter_func, c_scope_exit_func, c_return_func, c_inst_scope_enter_func, c_inst_scope_exit_func, c_inst_return_func, c_inst_counting_init_func, c_auto_gen_notes, c_int_atexit_func, c_inst_probe_includes
...
def get_scope_table(source: str, ranges: [], is_inst_counting=False) -> (str, dict):
    ...
    for global_index, i in enumerate(source):
        if in_struct_range(global_index): # Encountered struct range
            ...
            if i == '{':  # Encountered scope enter
                ...
                if is_inst_counting:
                    result_source += f'\n{c_inst_scope_enter_func}({new_scope.ID});\n'
                else:
                ...
                # Add scope exit prob
                if is_inst_counting:
                    result_source += f'\n{c_inst_scope_exit_func}({scope_stack[-1].ID});\n'
                else:
...
def add_return_stmt(source: str, is_inst_counting=False) -> str:
...
def inst_counting_process(file_loc: str) -> dict:
    """Preprocess work on input source code for dynamic instruction counting"""
    # Read the content of the source file
    with open(c_source_loc+file_loc, 'r') as src_file:
        content = src_file.read()
    ...
    # Insert prob to scope enter & exits
    content, scope_table = get_scope_table(content, struct_ranges, is_inst_counting=True)

    # Insert prob to return stmt
    content = add_return_stmt(content, is_inst_counting=True)
    ...
    content = content[:injecting_index] + c_int_atexit_func + c_inst_counting_init_func + content[injecting_index:]

    # Add includes back
    head = c_auto_gen_notes
    for include in includes:
        head += include + '\n'

    # Add additional includes
    for include in c_inst_probe_includes:
        head += include + '\n'

    # Concatenate with modified source
    content = head + content

    # Write the content to the destination file
    with open(c_altered_loc+file_loc.split('.')[0]+c_inst_counting_suffix, 'w') as dest_file:
        dest_file.write(content)
